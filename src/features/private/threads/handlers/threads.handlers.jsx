...
// Constants
import AdminConstants from "../constants/threads.constants";
import constants from "./constants/threads.handlers.constants";
// Libs
...
const submitHandlerEdit = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) =>
      prev.map((item) => (item._id === selectedRow._id ? data.thread : item))
    );
    toast.success(data.message);
  } catch (error) {
    if (error.response.data.errors) {
...
const fetchCategoriesHandler = async ({ setLoading, setCategories }) => {
  setLoading(true);
  try {
...
const ThreadsHandlers = ({
  ...
  return {
    ...
    handleFetchUsers: () => fetchUsersHandler({ setLoading, setUsers }),
  };
};
