...
// Constants
import AdminConstants from "../constants/comments.constants";
import constants from "./constants/comments.handlers.constants";
// Libs
...
const submitHandlerCreate = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) => [...prev, data.comment]);
...
const submitHandlerEdit = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) =>
      prev.map((item) => (item._id === selectedRow._id ? data.comment : item))
    );
    toast.success(data.message);
  } catch (error) {
    if (error.response.data.errors) {
...
const fetchThreadsHandler = async ({ setLoading, setThreads }) => {
  setLoading(true);
  try {
